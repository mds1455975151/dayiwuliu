...
public class ValidateCode {
    ...
    private String randString = "3456789ABCDEFGHJKLMNPQRSTUVWXY";//随机产生的字符串
    ...
    /**
     * 生成随机图片 baes64转码图片
     * 展示形式："data:image/png;base64,"+imageString
     */
    public VdCode getRandcode() {
        ...
        g.dispose();
        //传参
        VdCode vc = new VdCode();
        try {
        	ByteArrayOutputStream out = new ByteArrayOutputStream();  
        	ImageIO.write(image, "png", out);//将内存中的图片通过流动形式输出到客户端
        	byte[] b = out.toByteArray();
        	//验证码
        	vc.setCode(randomString);
        	//验证码图片
        	vc.setImageString(Base64.byteArrayToBase64(b));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return vc;
