 ...
 */
@Controller
@RequestMapping("/app/ownerreport")
public class AppOwnerReportAction {
	
	public Logger logger = LoggerFactory.getLogger(AppOwnerReportAction.class);
	...
	public AppResult detail(AppParam<ReportReq> appParam){
		...
		try {
			//拼装查询条件
			ReportReq query =appParam.getBody();
			ReportResp resp = billReportService.querReportDetail(query, true);
			appResult.setCode("000000");
