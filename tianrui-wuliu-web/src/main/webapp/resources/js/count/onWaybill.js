...
function index(No,flag){
	$.ajax({
		url : "/publicMember/rollingMessage/onWaybill",//
		data : {
			"pageNo":No,
			"pageSize":pageSize},
		type : "post",
...
function innerHTML(ret,flag){
	...
	for (var a = 0; a < data.length; a++) {
			...
			hml += ' <img src="'+trRoot+'/tianrui/images/hycar.png">';
			hml += '<span>'+data[a].vehicletypename+"—"+data[a].vehicleno+'</span>';
			hml += '</div>';
			hml += '<div class="fr">';
			...
			hml += '<span>'+data[a].cargoname+data[a].weight+data[a].measure+'</span>';
			hml += '</div>';
