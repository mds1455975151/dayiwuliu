package com.tianrui.app.web;

import java.io.BufferedReader;
import java.io.DataOutputStream;
import java.io.InputStreamReader;
import java.io.UnsupportedEncodingException;
import java.net.HttpURLConnection;
import java.net.URL;
import java.net.URLEncoder;
import java.util.ArrayList;
import java.util.List;

import com.alibaba.fastjson.JSON;
import com.tianrui.service.bean.alct.BodyConsigneeInfo;
import com.tianrui.service.bean.alct.BodyConsignorInfo;
import com.tianrui.service.bean.alct.BodyGoodsInfo;
import com.tianrui.service.bean.alct.BodyPriceInfo;
import com.tianrui.service.bean.alct.Shipments;
import com.tianrui.service.bean.alct.ShipmentsBody;
import com.tianrui.service.bean.alct.ShipmentsHead;

public class AlctBillApiTest {

	private static String url="https://oapi-staging.alct56.com/api/v1/openapi/shipments";
	
	
	static Shipments getParam2(){
		
		BodyConsigneeInfo consigneeInfo = new BodyConsigneeInfo();
		consigneeInfo.setConsignee("刘先生");
		consigneeInfo.setCustomerName("中国香港");
		consigneeInfo.setIdentificationNumberOfTheTaxpayer("91410482MA3XD9CA67");
		consigneeInfo.setTelephone("15236984523");
		
		BodyConsignorInfo consignorInfo = new BodyConsignorInfo();
		consignorInfo.setConsigner("李先生");
		consignorInfo.setCustomerName("中国大陆");
		consignorInfo.setIdentificationNumberOfTheTaxpayer("91330100799666058B");
		consignorInfo.setTelephone("15526354569");
		
		List<BodyGoodsInfo> goodsInfo = new ArrayList<BodyGoodsInfo>();
		BodyGoodsInfo goods = new BodyGoodsInfo();
		goods.setDescriptionOfGoods("木材");
		goods.setGoodsPropertyTypeCode(20);
		goods.setUnitWight(2000);
		goods.setUnitVol(0.0);
		goods.setUnit("车");
		goods.setQuantity(1);
		goodsInfo.add(goods);
		
		BodyPriceInfo priceInfo = new BodyPriceInfo();
		priceInfo.setPrice(500000);
		priceInfo.setMeasurementFunctionCode("WT");
		priceInfo.setPriceCurrencyCode("CNY");
		
		ShipmentsBody body = new ShipmentsBody();
		body.setShippingNoteNumber("YD09876");
		body.setConsignmentDateTime("2018-01-05T10:00:00");
		body.setBusinessTypeCode(10);
		body.setPickupLocation("文一西路969号");
		body.setPickupCountrySubdivisionCode(330110);
		body.setUnloadLocation("广成东路南侧天瑞大厦七楼");
		body.setUnloadCountrySubdivisionCode(410482);
		body.setDistance(958);
		body.setPlanPickupTimeStart("2018-01-05T10:00:00");
		body.setPlanPickupTimeEnd("2018-01-05T10:00:00");
		body.setPlanUnloadTimeStart("2018-01-05T10:00:00");
		body.setPlanUnloadTimeEnd("2018-01-05T10:00:00");
		body.setCargoTypeClassificationCode(10);
		body.setDriverIdentification("370832199310107878");
		body.setVehicleNumber("鲁N121212");
		body.setTotalWeight(5.5);
		body.setTotalCube(0.0);
		
		body.setPriceInfo(priceInfo);
		body.setGoodsInfo(goodsInfo);
		body.setConsignorInfo(consignorInfo);
		body.setConsigneeInfo(consigneeInfo);
		
		ShipmentsHead head = new ShipmentsHead();
		
		Shipments ship = new Shipments();
		ship.setBody(body);
		ship.setHead(head);
		return  ship;
	}
	
	static String  httpPost(String url,String param){
		StringBuffer sb = new StringBuffer("");
	   
	    try {
			URL realUrl = new URL(url);
	        // 打开和URL之间的连接
			HttpURLConnection conn = (HttpURLConnection)realUrl.openConnection();
	        // 设置通用的请求属性
			conn.addRequestProperty("Authorization", "Bearer eyJhbGciOiJSUzI1NiIsImtpZCI6IjRDNTQwNzlDQTU4OUZDRUREMTg5NzYzRUZBQTU4MTUzNTQ5MUNCOEMiLCJ0eXAiOiJKV1QiLCJ4NXQiOiJURlFIbktXSl9PM1JpWFktLXFXQlUxU1J5NHcifQ..AOK0XGoWZhfP2sPQI9G3y8UO6sebKjZH2tvMzThNtJLccaPIOzNKSnQXlP9eWdn__LENQuryInXOfvQ-Rl7hU_VJZ1qVGvfBVdd2l1Ljz4jZtGfcmO1LGytgKMpVkmEblwmuk28i07r6ywTosOUmDZRmDI8TykFOVfZMRxof0h2F4SHIHwzCeSrgq0iV39Os6d_683A1TihLM3SxyfArjzPPKFTAGIzf6bbSFVbKdfZN9R4gSiKDU5Mtl2bAVAsr3gVx1OSVMnMBBw1UW9f8HmI0Fleyj7sV6ufIFJmsghPHxwuMkWgUJy5nPymxzT7JkhpHT4CniYXOi8lKKiDHXLHkHWcv2G7PeE_VTUPHw2MbcsR2zm09YVX-DzFWYcxtJLVl7wxXqOes9CK2KqQHmelOqVKc34iFQkqCztOzp0yEb-A2xck1kI_zdZFUALjed3C28qvxk52kDlVxlFgGQNXOi0BDSYlQAdvk6DUjQMfMeMYETdcRhBFVx_fuQD0w5kajtO8wDq3OaobSAkoT7eHOqS_wSD36pcs6AyaRNYJHZjpaV-CrNvAW1qxq3Rm-ZqygDYTX0FASr7cR07WyzP03aQH8w7HGBiTe_dNn0QVqT-oidieWuAaIfV2kc2NKRBAk3-gI399NOFgpxKOjXsRpfoYIVD2JguYSJ9G7c4Q");
			conn.setRequestMethod("POST");
			conn.setUseCaches(false);
			conn.setInstanceFollowRedirects(true);
			conn.setRequestProperty("Content-Type","application/json");
			conn.setRequestProperty("Accept-Charset", "UTF-8");
			
	        // 发送POST请求必须设置如下两行     
	        conn.setDoOutput(true);
	        conn.setDoInput(true);
	        //连接
	        conn.connect();
	        // 获取URLConnection对象对应的输出流
	        DataOutputStream out = new DataOutputStream(conn.getOutputStream());
	        out.write(param.getBytes("utf-8"));
	        // flush输出流的缓冲
	        out.flush();
            out.close();
	        // 定义BufferedReader输入流来读取URL的响应
            BufferedReader reader = new BufferedReader(new InputStreamReader(conn.getInputStream()));
            String lines="";
            while ((lines = reader.readLine()) != null) {
                lines = new String(lines.getBytes("utf-8"), "utf-8");
                sb.append(lines);
            }
            System.out.println(sb);
            reader.close();
            // 断开连接
            conn.disconnect();
	    }catch(Exception e){
	    	System.out.println("异常信息"+e.getMessage());
	    }
	    return sb.toString();
	}
	public static void main(String[] args) throws UnsupportedEncodingException {
		String param =JSON.toJSONString(getParam2());
		System.out.println(param);
		System.out.println(url);
		System.out.println(httpPost(url,param));
	}
	
	
}
